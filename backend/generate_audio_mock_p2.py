...
# Mock Test - Listen to a Conversation. Same splicing approach as generate_audio_p2.py,
# reading from the mock-only JSON and writing to a separate audio folder.
...
with open('mock_listening_conversation.json', 'r', encoding='utf-8') as f:
    conversations = json.load(f)

os.makedirs('audio/mock_listening_p2', exist_ok=True)
...
async def generate_all():
    with tempfile.TemporaryDirectory() as tmp:
        ...
        for convo in conversations:
            out_path = f"audio/mock_listening_p2/{convo['id']}.mp3"
