  ...
  return (
    <div style={{ display: 'flex', height: '100vh', width: '100vw', fontFamily: 'sans-serif', backgroundColor: '#f4f6fa', overflow: 'hidden' }}>
      ...
      <div style={{ flex: 1, padding: '18px 20px', overflow: 'hidden', display: 'flex', flexDirection: 'column' }}>
        ...
        {/* DASHBOARD */}
        {currentTab === 'dashboard' && (
          <div style={{ display: 'grid', gridTemplateColumns: '1.4fr 1fr 1fr', gap: '14px', flex: 1, minHeight: 0 }}>

            {/* SOL: Scores */}
            <div style={{ background: '#fff', borderRadius: '12px', padding: '18px', border: '0.5px solid #e1e4ed', display: 'flex', flexDirection: 'column' }}>
            ...
            <div style={{ display: 'flex', flexDirection: 'column', gap: '12px', minHeight: 0 }}>
              <div style={{ background: '#fff', borderRadius: '12px', padding: '16px', border: '0.5px solid #e1e4ed' }}>
                ...
                <div style={{ display: 'flex', alignItems: 'center', gap: '10px' }}>
                  ...
                  <div>
                    ...
                    <input type="date" value={examDate} onChange={e => setExamDate(e.target.value)} style={{ marginTop: '6px', fontSize: '11px', padding: '3px 6px', borderRadius: '6px', border: '0.5px solid #cbd5e1', background: '#f4f6fa', color: '#11162d' }} />
                  </div>
                </div>
              </div>
              ...
              {[
                ...
                <div key={item.name} style={{ background: '#fff', borderRadius: '10px', padding: '12px 14px', border: '0.5px solid #e1e4ed', display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
                  <div style={{ fontSize: '11px', color: '#616473' }}>{item.name}</div>
                  <div style={{ textAlign: 'right' }}>
                    <div style={{ fontSize: '17px', fontWeight: '600' }}>{item.score}</div>
            ...
            {/* SAĞ: Goals */}
            <div style={{ background: '#fff', borderRadius: '12px', padding: '16px', border: '0.5px solid #e1e4ed', display: 'flex', flexDirection: 'column' }}>
              <div style={{ fontSize: '12px', fontWeight: '600', marginBottom: '12px' }}>Today's goals</div>
              <div style={{ display: 'flex', flexDirection: 'column', gap: '10px', flex: 1 }}>
                {examDaysLeft === null ? (
                  <div style={{ fontSize: '12px', color: '#999', lineHeight: '1.5' }}>Select an exam date to generate your personalized daily goals.</div>
                ) : generateGoals(examDaysLeft, userData).map((g, i) => (
                  <div key={i} style={{ display: 'flex', alignItems: 'flex-start', gap: '8px', fontSize: '12px', color: '#444', lineHeight: '1.5' }}>
                    <span style={{ color: '#701fa1', marginTop: '1px', flexShrink: 0 }}>○</span> {g}
                  </div>
                ))}
              </div>
            </div>
